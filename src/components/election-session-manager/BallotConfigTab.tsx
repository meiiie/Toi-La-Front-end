...
interface BallotConfigTabProps {
  ...
  initialMetadata?: BallotMetadata; // Thêm prop để nhận metadata từ component cha
}
...
const BallotConfigTab: React.FC<BallotConfigTabProps> = ({
  ...
  useEffect(() => {
    if (initialMetadata) {
      // Sử dụng metadata từ prop nếu có
      setBallotName(initialMetadata.name || '');
      setBallotDescription(initialMetadata.description || '');
      setBallotImageUrl(initialMetadata.image || '');
      setBallotBgColor(
        initialMetadata.background_color ? `#${initialMetadata.background_color}` : '#f8f9fa',
      );
      ...
      setIs3DModel(
        initialMetadata.image?.endsWith('.glb') ||
          initialMetadata.image?.endsWith('.gltf') ||
          initialMetadata.image?.toLowerCase().includes('.glb') ||
          initialMetadata.image?.toLowerCase().includes('.gltf') ||
          false,
      );

      if (initialMetadata.attributes && Array.isArray(initialMetadata.attributes)) {
        setAttributes(initialMetadata.attributes);
      }
    } else if (selectedSession?.id) {
      // Nếu không có initialMetadata, thử tải từ localStorage như cũ
      try {
  ...
  }, [selectedSession?.id, initialMetadata]);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: 'Cấu hình phiếu bầu đã được lưu và sẽ áp dụng khi cấp phiếu cho cử tri',
        });
      }, 600);
    } catch (error) {
      ...
      <CardContent className="space-y-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="basic" className="space-y-6 animate-in fade-in-50 duration-300">
            ...
            {/* Thêm note về liên kết với cấp phiếu */}
            <Alert className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 mt-2">
              <Info className="h-4 w-4 text-blue-600 dark:text-blue-400" />
              <AlertDescription>
                Cấu hình này sẽ được sử dụng khi cấp phiếu bầu cho cử tri. Đảm bảo lưu cấu hình sau
                khi thay đổi.
              </AlertDescription>
            </Alert>
          </TabsContent>
